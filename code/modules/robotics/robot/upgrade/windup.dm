/obj/item/roboupgrade/windup
	...
	proc/start_charging(mob/user, mob/living/silicon/robot/target)
		...
		if (C.charge == C.maxcharge)
			...
			actions.start(new /datum/action/bar/icon/windup_action(src, user, target, src.windup_duration), user)
...
/datum/action/bar/icon/windup_action
	...
	icon_state = "up-windup"
	var/obj/item/roboupgrade/windup/upgrade
	var/mob/living/user
	var/mob/living/silicon/robot/target
	var/obj/item/cell/C

	New(upgrade, user, target, duration)
		. = ..()
		src.upgrade = upgrade
		src.user = user
		src.target = target
		src.duration = duration
		src.C = src.target.cell
		src.icon = src.upgrade.icon
		src.icon_state = src.upgrade.icon_state

	proc/checkContinue()
		if(!src.C || src.C.charge >= src.C.maxcharge || BOUNDS_DIST(src.user, src.target) > 0 || src.user == null || src.target == null)
			return FALSE
		return TRUE
	...
	loopStart()
		..()
		src.C.give(upgrade.charge_rate)
